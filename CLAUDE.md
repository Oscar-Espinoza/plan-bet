...
**Slant.** Concrete ground, ink wedges, volt plates, condensed Archivo headings, IBM Plex Sans body, and DM Mono figures. Values and surface rules live in `docs/design/slant-tokens.md`; migration and validation details in `docs/design/slant-migration.md`. Main palette families: concrete (ground/raised/sunk), ink (text and dark surfaces), chalk (reversed text), rules, volt, and settled outcomes (field/clay). Use `--ink-soft` for secondary text on sunk surfaces; `--ink-faint` only on concrete or raised surfaces. Necessary control outlines use ink tokens, not decorative hairlines.

Volt is a fill on concrete and may be foreground only inside ink. The clock is the loudest figure; balances, stakes, prices, and returns stay ink on concrete or chalk on ink. Field/clay are for settled outcomes. Archivo headings are uppercase, width 78 and weight 800 for large names, width 108 for labels. Every figure uses DM Mono with tabular numerals. Nothing rounded (except the decorative empty-state circle), no gradients, glass, glow, chat bubbles or sparkles.

Plates use `skewX(-10deg)` with counter-skewed content. Inputs remain upright. Compose the existing 1px press movement with the skew; preserve entry timing and reduced motion. Wedges and the bottom ribbon may use clip-path because they are borderless. Leave content room for the wedge drop and ribbon rise. Keep the scorebug's decorative club stripe from canonical team data.

`AppShell` owns ribbon portal targets; route components own their content and state. `BetSlip` portals returns, the native submit button (associated by form ID), and announced feedback. The board alone portals the displayed next fixture's countdown. Mobile shell rows are workspace, ribbon, navigation; desktop navigation stays in the header and the ribbon is fixed below. `ResizeObserver` measures ribbon/navigation/tour heights for overlays and content clearance. Only the bottommost band adds safe-area padding. Dates still render in the browser timezone via `local-date-time.tsx`. Fictional credits remain non-withdrawable; prices and markets come from the existing house table.
